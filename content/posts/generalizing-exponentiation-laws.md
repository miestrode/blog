...
Alongside exponentiation, students usually learn the exponentiation laws, which are certain identities governing the operation.
...
* Product law: $\left(a^b\right)^c = a^{b \times c}$
* Distributive law: $\left(a \times b\right)^c = a^c \times b^c$
* Division law: $\frac{a^c}{b^c}=\left(\frac{a}{b}\right)^c$
* ~~Commutative law: $a^b = b^a$~~
* ~~Associative law: $a^{\left(b^c\right)} = \left(a^b\right)^c$~~
* ~~Distributive law 2 - Electric Boogalo: $\left(a + b\right)^n = a^n + b^n$~~
...
$$a^b \times a^c = \underbrace{\left(a \times a \times \dots \times a \times a\right)}\_{\text{$b$ times}} \times \underbrace{\left(a \times a \times \dots \times a \times a\right)}_{\text{$c$ times}}$$

However, look! There is no real separator between the two groups of products in here, so we should be able to say this expression equals:
$$\underbrace{a \times a \times \dots \times a \times a \times a \times a \times \dots \times a \times a}_{\text{$b + c$ times}}$$

Now notice again, by our original definition, this is the same as writing $a^{b + c}$. As required!
...
to clairify this is a definition, not an "equality" or identity by the traditional sense.
...
"When are the other definitions (such as $\lim_{n \to \infty} \left(1 + \frac{1}{n}\right)^n$) useful?".
...
"Wait, what? A sum of sums?" Indeed. This might look intimidating at first, so it might be worth it to take some time to understand what is being written here.
...
A more general version of it works for expressions of the form $\left(a_1 + a_2 + \dots + a_{n - 1} + a_n\right)\left(b_1 + b_2 + \dots + b_{n - 1} + b_n\right)$, and turns them into:

$$\sum_{i = 0}^n \sum_{j = 0}^n a_i b_j$$

A first thought might be to use something in the spirit of this identity, but this quickly fails, as we get a resulting sum that looks like
...
(and that we visit all combinations, although this can be done quite quickly, and will not be shown here). As for how we show that, this turns out to be quite short to prove.

If we fix a particular value for $k$ (in the Cauchy Product, far above), letting only $n$ change, then for every $n$, each second index, $n - k$ is unique.
...
$$e^x e^y = \left(\sum_{i = 0}^\infty \frac{x^i}{i!}\right)\left(\sum_{j = 0}^\infty \frac{x^j}{j!}\right)$$

Now, by the Cauchy product, we get:

$$\sum_{n = 0}^\infty \sum_{k = 0}^n \frac{x^k y^{n - k}}{k!\left(n - k\right)!}$$
...
$$\sum_{n = 0}^\infty \frac{1}{n!} \sum_{k = 0}^n \frac{n! x^k y^{n - k}}{k!\left(n - k\right)!}
...
$$\left(x + y\right)^n = \underbrace{xx \dots xx}\_{\text{$n$ times}} +
...
two $x$s, now times $x^2$, three $x$s, times $x^3$ and let's not forget, zero $x$s (technically, we were also picking ones, but they don't matter in our product). 
...
So $\exp(n)$, where $n$ is a natural number, is a product $e$s. $n$ of them to be exact.
...
This definition, is of general exponentation, which we have to define, otherwise identities such as ${\left(e^x\right)}^y = e^{xy}$ won't be proveable, because they aren't even defined.
...
### Product law
By our definition of real exponentation, and the definition of the natural logarithm, we get: $\left(a^b\right)^c = e^{c \ln {e ^{b \ln a}}} = e^{bc \ln a}$.
Finally, by our same exponentation definition we get: $a^{bc}$.

### Sum law
A proof the sum law is fairly short as well. First, notice that: $a^b a^c = e^{b \ln a} e^{c \ln a}$.
Then, using our fundemental property of exponentation and the exponentation definition, we get: $e^{b \ln a + c \ln a} = e^{\left(b + c\right) \ln a} = a^{b + c}$.
...
Now, to finish, remember that logarithm law in start of this section? Let's use it: $e^{c \ln ab} = \left(ab\right)^c$. This ends our proof.
...
undefined. It might make you wonder why that's the case, and drive you to see how that effects the laws laid out in here.
